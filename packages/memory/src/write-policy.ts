...
import { WritePolicyError, ProfileKeyNotAllowedError } from './errors.js';
...
export class WritePolicyEngine {
  private config: WritePolicyConfig;
  ...
  validateProfileKey(key: string): void {
    if (this.config.profileKeyWhitelist === null) {
      return; // All keys allowed
    }

    if (!this.config.profileKeyWhitelist.includes(key as ProfileKey)) {
      throw new ProfileKeyNotAllowedError(key, this.config.profileKeyWhitelist);
    }
  }
  ...
  decideWrite(event: MemoryEvent): WriteDecision {
    ...
    } else if (event.type === 'TOOL_RESULT' && this.isHighValueResult(event)) {
      targetLayers.push('semantic');
      confidence = 0.7;
    ...
    if (this.containsPreference(event)) {
    ...
    const shouldWrite = targetLayers.length > 0 && confidence >= this.config.minConfidence;
  ...
  calculateDecay(timestamp: number): number {
    ...
    return Math.pow(this.config.timeDecayFactor, ageDays);
  }
  ...
  shouldSummarize(eventCount: number, tokenCount: number): {
    ...
    if (eventCount >= this.config.summarizeEveryNEvents) {
      return {
        should: true,
        reason: `Event count threshold reached: ${eventCount} >= ${this.config.summarizeEveryNEvents}`,
      };
    }

    if (tokenCount >= this.config.summarizeTokenThreshold) {
      return {
        should: true,
        reason: `Token threshold reached: ${tokenCount} >= ${this.config.summarizeTokenThreshold}`,
      };
    }
   ...
   */
  private isHighValueResult(event: MemoryEvent): boolean {
   ...
   */
  private containsPreference(event: MemoryEvent): boolean {
