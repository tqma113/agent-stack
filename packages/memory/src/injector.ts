...
/**
 * Memory Injector - formats MemoryBundle for prompt injection
 */
export class MemoryInjector {
  private template: string;

  constructor(template: string = DEFAULT_INJECTION_TEMPLATE) {
  ...
  inject(bundle: MemoryBundle, options: InjectionOptions = {}): string {
    ...
    if (includeProfile && bundle.profile.length > 0) {
      sections.profile = this.formatProfile(bundle.profile);
      flags.hasProfile = true;
    }

    // Task state section
    if (includeTaskState && bundle.taskState) {
      sections.taskState = this.formatTaskState(bundle.taskState);
      flags.hasTaskState = true;
    }

    // Summary section
    if (includeSummary && bundle.summary) {
      sections.summary = this.formatSummary(bundle.summary);
      flags.hasSummary = true;
    }

    // Recent events section
    if (includeEvents && bundle.recentEvents.length > 0) {
      sections.events = this.formatEvents(bundle.recentEvents);
      flags.hasEvents = true;
    }

    // Semantic chunks section
    if (includeChunks && bundle.retrievedChunks.length > 0) {
      sections.chunks = this.formatChunks(bundle.retrievedChunks);
      flags.hasChunks = true;
    }

    // Warnings section
    if (includeWarnings && bundle.warnings.length > 0) {
      sections.warnings = this.formatWarnings(bundle.warnings);
      flags.hasWarnings = true;
    }

    return this.renderTemplate(options.template || this.template, {
      ...sections,
      ...flags,
    });
  }
   ...
   */
  private formatProfile(items: ProfileItem[]): string {
    return items
      .map((item) => {
   ...
   * Format task state
   */
  private formatTaskState(state: TaskState): string {
    const lines: string[] = [];
   ...
   */
  private formatEvents(events: MemoryEvent[]): string {
    return events
      .map((event) => {
   ...
   */
  private formatChunks(results: SemanticSearchResult[]): string {
    return results
      .map((result, index) => {
        const source = result.chunk.sourceType || 'unknown';
        const score = result.score.toFixed(2);
        const text = this.truncate(result.chunk.text, 200);
   ...
   * Format warnings
   */
  private formatWarnings(warnings: MemoryBundle['warnings']): string {
    return warnings
      .map((warning) => `⚠️ [${warning.type}] ${warning.message}`)
      .join('\n');
  }
